...
| `<서식18>` | `SELF_RESOLUTION` | `<서식12>` | 전담기구 보고서: 자체해결 판단 사안내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 `양측 간 사과`, `신체적/정신적 피해 경미`, `지속적이거나 반복적으로 보기 어려운 점`, `보복행위가 없었던 점`을 포함하며, `학교폭력 예방 및 대책에 관한 법률 상 학교장 자체해결 요건에 해당하는 것으로 판단됨` 문구로 마무리 | 500자 이내 |
...
| `<서식19>` | `CLOSURE_*` | `<서식12>` | 전담기구 보고서: 종결 사안내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 `오인신고`, `의심사안`, `관련자 성인 등 특정 불가` 중 선택 사유와 `학교폭력 예방 및 대책에 관한 법률 상 학교장 종결 처리에 해당하는 것으로 판단됨` 취지로 마무리 | 500자 이내 |
| `<서식20>` | `SELF_RESOLUTION` | `<서식12>` + `<서식18>` | 자체해결 동의서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 보호자가 읽는 문서에 맞게 `<서식18>`의 자체해결 판단 근거를 완곡하고 중립적인 표현으로 재서술. 상대방 보호자가 보더라도 불필요한 논란이 적은 문체 유지 | 400자 이내 |
| `<서식21>` | `CLOSURE_*` | `<서식12>` + `<서식19>` | 종결처리 동의서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 보호자가 읽는 문서에 맞게 `<서식19>`의 종결 판단 근거를 완곡하고 중립적인 표현으로 재서술 | 400자 이내 |
| `<서식22>` | `SELF_RESOLUTION` | `<서식12>` + `<서식18>` | 학교장 자체해결 결과 보고서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 자체해결 흐름의 최종 보고서에 붙여넣을 수 있도록 `<서식18>`의 자체해결 판단을 종합해 행정문체로 서술 | 400자 이내 |
| `<서식22>` | `CLOSURE_*` | `<서식12>` + `<서식19>` | 학교장 종결 결과 보고서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 종결 흐름의 최종 보고서에 붙여넣을 수 있도록 `<서식19>`의 종결 판단을 종합해 행정문체로 서술 | 400자 이내 |
...
`<서식18~22>`는 모두 `전술: 사안조사 보고서 요약` + `후술: Draft별 판단 또는 안내 문구` 구조를 따른다. 이 구조는 MockGenerator뿐 아니라 이후 Gemini/OpenAI provider의 prompt와 output harness에도 그대로 적용한다.
...
후술: 선택된 Draft 흐름에 따른 판단, 동의서 안내, 결과 보고 문구
```
...
|---|---|---|---|---|
| DRAFT 18 / `<서식18>` | `SELF_RESOLUTION` | `<서식12>` 사안조사 보고서 | 전담기구 보고서의 `자체해결 또는 심의위원회 요청` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 개조식에 가까운 행정기관 문체로 작성하며 `양측 간 사과`, `신체적/정신적 피해 경미`, `지속적이거나 반복적으로 보기 어려운 점`, `보복행위가 없었던 점`을 빠뜨리지 않는다. 결론 문장은 `학교폭력 예방 및 대책에 관한 법률 상 학교장 자체해결 요건에 해당하는 것으로 판단됨` 취지로 닫는다. |
...
| DRAFT 19 / `<서식19>` | `CLOSURE_FALSE_REPORT`, `CLOSURE_SUSPECTED_CASE`, `CLOSURE_ADULT_OR_UNIDENTIFIABLE` | `<서식12>` 사안조사 보고서 | 전담기구 보고서(종결)의 `종결 시(오인신고 등)` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 선택된 종결 사유를 문장 안에 명시하고, 오인신고, 의심사안, 관련자 성인 등 특정 불가 중 선택된 사유와 조사 내용을 연결하며, 결론은 `학교폭력 예방 및 대책에 관한 법률 상 학교장 종결 처리에 해당하는 것으로 판단됨` 취지로 정리한다. |
| DRAFT 20 / `<서식20>` | `SELF_RESOLUTION` | `<서식12>` 사안조사 보고서 + DRAFT 18 생성문 | 자체해결 동의서의 `보호자 전달 통지서 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 보호자에게 전달되는 문서이므로 DRAFT 18의 판단 근거를 그대로 복사하지 않고 학생 간 관계 회복, 피해 정도, 지속성, 보복행위 부재를 더 완곡하고 중립적인 표현으로 풀어 쓴다. 상대방 보호자가 읽어도 단정적 비난으로 보이지 않도록 표현한다. |
| DRAFT 21 / `<서식21>` | `CLOSURE_FALSE_REPORT`, `CLOSURE_SUSPECTED_CASE`, `CLOSURE_ADULT_OR_UNIDENTIFIABLE` | `<서식12>` 사안조사 보고서 + DRAFT 19 생성문 | 종결처리 동의서의 `보호자 전달 통지서 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 보호자에게 전달되는 문서이므로 DRAFT 19의 종결 판단 근거를 완곡하게 재서술한다. 선택된 종결 사유는 반영하되, 학생 또는 보호자를 비난하거나 법적 책임을 단정하는 표현은 피한다. |
| DRAFT 22 / `<서식22>` | `SELF_RESOLUTION` | `<서식12>` 사안조사 보고서 + DRAFT 18 생성문 | 학교장 자체해결 결과 보고서의 `사안조사 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 최종 결과 보고서에 붙여넣을 수 있는 행정문체로 DRAFT 18의 자체해결 판단을 바탕으로 판단 근거와 자체해결 흐름을 400자 이내로 종합한다. |
| DRAFT 22 / `<서식22>` | `CLOSURE_FALSE_REPORT`, `CLOSURE_SUSPECTED_CASE`, `CLOSURE_ADULT_OR_UNIDENTIFIABLE` | `<서식12>` 사안조사 보고서 + DRAFT 19 생성문 | 학교장 종결 결과 보고서의 `사안조사 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 최종 종결 결과 보고서에 붙여넣을 수 있는 행정문체로 선택된 종결 사유와 DRAFT 19의 종결 판단을 400자 이내로 종합한다. |
