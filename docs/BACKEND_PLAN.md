...
|---|---|---|---|---|---|
| `<서식10>` | 사안접수 | 사용자가 두서없이 입력한 사실 확인내용 | 사안접수 보고용 `발생 경위` | 사실 확인내용을 행정문체로 정리하되 존대어를 쓰지 않고, 확정 판단이 아니라 접수 단계 초안으로 작성. 프론트 복사 블록은 `사안 개요`, `조치 사항`을 제외하고 `발생 경위`만 표시 | 1,000자 이내 |
| `<서식12>` | 조사관 입력 | 외주 조사관 입력 | 이후 서식들의 핵심 source | 원문 저장. 후속 Draft 18~22의 1차 근거로 사용 | 원문 저장 |
| `<서식18>` | `SELF_RESOLUTION` | `<서식12>` | 전담기구 보고서: 자체해결 판단 사안내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 `양측 간 사과`, `신체적/정신적 피해 경미`, `지속적이거나 반복적으로 보기 어려운 점`, `보복행위가 없었던 점`을 포함하며, `학교폭력 예방 및 대책에 관한 법률 제13조의2 제1항`에 따른 학교장 자체해결 요건 해당 취지로 마무리 | 500자 이내 |
| `<서식18>` | `COMMITTEE_REQUEST` | `<서식12>` | 전담기구 보고서: 심의위원회 요청 사안내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 치료 진단서, 재산상 피해, 지속성, 보복행위, 피해관련 학생의 심의위원회 요청 의사 중 1개 이상을 근거로 심의 요청 필요성을 행정문체로 서술 | 500자 이내 |
| `<서식19>` | `CLOSURE_*` | `<서식12>` | 전담기구 보고서: 종결 사안내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 `오인신고`, `의심사안`, `관련자 성인 등 특정 불가` 중 선택 사유와 `학교폭력이 아닌 사안 등 종결처리 대상에 해당하는 것으로 판단됨` 취지로 마무리. `제13조`, `제13조의2`, `제14조` 계열은 절대 쓰지 않으며, `오인신고`/`의심사안`은 `학교폭력예방 및 대책에 관한 법률 제2조 제1항`, `관련자 성인 등 특정 불가`는 `학교폭력예방 및 대책에 관한 법률 제2조 제3항`만 근거로 사용 | 500자 이내 |
| `<서식20>` | `SELF_RESOLUTION` | `<서식12>` + `<서식18>` | 자체해결 동의서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 `<서식18>`의 `학교폭력 예방 및 대책에 관한 법률 제13조의2 제1항`에 따른 자체해결 판단 근거를 조금 더 완곡한 행정문체로 재서술. 학부모에게 제안하거나 동의를 요청하지 않고 사안조사 내용만 정리하며, 존대어와 요청형 문장은 쓰지 않음 | 400자 이내 |
| `<서식21>` | `CLOSURE_*` | `<서식12>` + `<서식19>` | 종결처리 동의서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 `<서식19>`의 종결 판단 근거를 조금 더 완곡한 행정문체로 재서술. `학교폭력이 아닌 사안 등 종결처리` 흐름과 허용 법령 근거를 유지하고, `제13조`, `제13조의2`, `제14조` 계열, 학부모 제안, 동의 요청, 존대어, 요청형 문장은 쓰지 않음 | 400자 이내 |
| `<서식22>` | `SELF_RESOLUTION` | `<서식12>` + `<서식18>` | 학교장 자체해결 결과 보고서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 자체해결 흐름의 최종 보고서에 붙여넣을 수 있도록 `<서식18>`의 `학교폭력 예방 및 대책에 관한 법률 제13조의2 제1항`에 따른 자체해결 판단을 종합해 행정문체로 서술 | 400자 이내 |
| `<서식22>` | `CLOSURE_*` | `<서식12>` + `<서식19>` | 학교장 종결 결과 보고서: 사안조사 내용 | 전술은 `<서식12>`의 핵심 사실관계 요약으로 시작하고, 후술은 종결 흐름의 최종 보고서에 붙여넣을 수 있도록 `<서식19>`의 종결 판단과 허용 법령 근거를 종합해 행정문체로 서술. `제13조`, `제13조의2`, `제14조` 계열은 쓰지 않음 | 400자 이내 |
...
`<서식10>`과 `<서식18~22>`는 모두 존대어를 쓰지 않고 행정문체를 필수로 사용한다. `<서식10>`은 복사 블록을 `발생 경위` 하나로 정리한다. `<서식18~22>`는 `전술: 사안조사 보고서 요약` + `후술: Draft별 판단 또는 사안조사 내용 정리` 구조를 따른다. 이 구조는 MockGenerator뿐 아니라 이후 Gemini/OpenAI provider의 prompt와 output harness에도 그대로 적용한다.
...
- 문구 생성 규칙을 prompt/mock template으로 고정한다.
  - DRAFT 18 + `SELF_RESOLUTION`:
    ...
    - `학교폭력 예방 및 대책에 관한 법률 제13조의2 제1항`에 따른 학교장 자체해결 요건 해당 문구를 포함한다.
  - DRAFT 18 + `COMMITTEE_REQUEST`:
    - 사안조사 보고서를 참고해 치료 진단서, 재산상 피해, 지속성, 보복행위, 피해관련 학생의 심의위원회 요청 의사 중 1개 이상을 근거로 서술한다.
  - DRAFT 19 + `CLOSURE_*`:
    - 선택한 사유를 명시하고, `학교폭력이 아닌 사안 등 종결처리 대상에 해당하는 것으로 판단됨` 취지로 서술한다.
    - `제13조`, `제13조의2`, `제14조` 계열은 절대 쓰지 않는다.
    - `오인신고`, `의심사안`은 `학교폭력예방 및 대책에 관한 법률 제2조 제1항`만 근거로 쓴다.
    - `관련자 성인 등 특정 불가`는 `학교폭력예방 및 대책에 관한 법률 제2조 제3항`만 근거로 쓴다.
  - DRAFT 20:
    - `SELF_RESOLUTION`일 때만 활성화하며, 사안조사 보고서와 DRAFT 18 내용을 바탕으로 조금 더 완곡한 행정문체로 사안조사 내용을 정리한다. 학부모에게 제안하거나 동의를 요청하지 않고, 존대어와 요청형 문장은 쓰지 않는다.
  - DRAFT 21:
    - `CLOSURE_*`일 때만 활성화하며, 사안조사 보고서와 DRAFT 19 내용을 바탕으로 조금 더 완곡한 행정문체로 사안조사 내용을 정리한다. 학부모에게 제안하거나 동의를 요청하지 않고, 존대어와 요청형 문장은 쓰지 않는다.
  - DRAFT 22:
...
특히 DRAFT 18~22는 모두 두 문단 또는 두 문장 흐름을 기본으로 한다.

```text
전술: <서식12> 사안조사 보고서의 핵심 사실관계 요약
후술: 선택된 Draft 흐름에 따른 판단, 사안조사 내용 정리, 결과 보고 문구
```

정식 AI provider를 붙여도 이 구조는 프롬프트와 검증 하네스에서 강제한다. 단순히 `행정문체로 정리하라`, `완곡하게 작성하라` 같은 지시를 모델에 전달하는 데서 끝내지 않고, 결과문이 실제로 사안조사 요약을 선행하고 Draft별 판단을 후술하는지 검사해야 한다.

| Draft | 활성 흐름 | 사안 source | 생성 필드 | 완성본 서술 방식 |
|---|---|---|---|---|
| DRAFT 18 / `<서식18>` | `SELF_RESOLUTION` | `<서식12>` 사안조사 보고서 | 전담기구 보고서의 `자체해결 또는 심의위원회 요청` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 개조식에 가까운 행정기관 문체로 작성하며 `양측 간 사과`, `신체적/정신적 피해 경미`, `지속적이거나 반복적으로 보기 어려운 점`, `보복행위가 없었던 점`을 빠뜨리지 않는다. 결론 문장은 `학교폭력 예방 및 대책에 관한 법률 제13조의2 제1항`에 따른 학교장 자체해결 요건 해당 취지로 닫는다. |
| DRAFT 18 / `<서식18>` | `COMMITTEE_REQUEST` | `<서식12>` 사안조사 보고서 | 전담기구 보고서의 `자체해결 또는 심의위원회 요청` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 심의위원회 요청 사유를 행정문체로 정리하며 치료 진단서, 재산상 피해, 지속성, 보복행위, 피해관련 학생의 심의위원회 요청 의사 중 확인된 요소를 1개 이상 근거로 삼는다. 자체해결 요건 문구를 사용하지 않는다. |
| DRAFT 19 / `<서식19>` | `CLOSURE_FALSE_REPORT`, `CLOSURE_SUSPECTED_CASE`, `CLOSURE_ADULT_OR_UNIDENTIFIABLE` | `<서식12>` 사안조사 보고서 | 전담기구 보고서(종결)의 `종결 시(오인신고 등)` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 선택된 종결 사유를 문장 안에 명시하고, 오인신고, 의심사안, 관련자 성인 등 특정 불가 중 선택된 사유와 조사 내용을 연결하며, 결론은 `학교폭력이 아닌 사안 등 종결처리 대상에 해당하는 것으로 판단됨` 취지로 정리한다. 제13조, 제13조의2, 제14조 계열은 쓰지 않고, 오인신고/의심사안은 제2조 제1항, 관련자 성인 등 특정 불가는 제2조 제3항만 근거로 쓴다. |
| DRAFT 20 / `<서식20>` | `SELF_RESOLUTION` | `<서식12>` 사안조사 보고서 + DRAFT 18 생성문 | 자체해결 동의서의 `보호자 전달 통지서 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 DRAFT 18의 자체해결 판단 근거를 조금 더 완곡한 행정문체로 재서술한다. 학부모에게 제안하거나 동의를 요청하지 않고 사안조사 내용만 정리하며, 존대어와 요청형 문장은 쓰지 않는다. |
| DRAFT 21 / `<서식21>` | `CLOSURE_FALSE_REPORT`, `CLOSURE_SUSPECTED_CASE`, `CLOSURE_ADULT_OR_UNIDENTIFIABLE` | `<서식12>` 사안조사 보고서 + DRAFT 19 생성문 | 종결처리 동의서의 `보호자 전달 통지서 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 DRAFT 19의 종결 판단 근거와 허용 법령 근거를 조금 더 완곡한 행정문체로 재서술한다. `학교폭력이 아닌 사안 등 종결처리` 흐름을 유지하고, 제13조, 제13조의2, 제14조 계열, 학부모 제안, 동의 요청, 존대어, 요청형 문장은 쓰지 않는다. |
| DRAFT 22 / `<서식22>` | `SELF_RESOLUTION` | `<서식12>` 사안조사 보고서 + DRAFT 18 생성문 | 학교장 자체해결 결과 보고서의 `사안조사 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 최종 결과 보고서에 붙여넣을 수 있는 행정문체로 DRAFT 18의 `학교폭력 예방 및 대책에 관한 법률 제13조의2 제1항`에 따른 자체해결 판단을 바탕으로 판단 근거와 자체해결 흐름을 400자 이내로 종합한다. |
| DRAFT 22 / `<서식22>` | `CLOSURE_FALSE_REPORT`, `CLOSURE_SUSPECTED_CASE`, `CLOSURE_ADULT_OR_UNIDENTIFIABLE` | `<서식12>` 사안조사 보고서 + DRAFT 19 생성문 | 학교장 종결 결과 보고서의 `사안조사 내용` | 전술은 `<서식12>` 핵심 사실관계 요약으로 시작한다. 후술은 최종 종결 결과 보고서에 붙여넣을 수 있는 행정문체로 선택된 종결 사유, DRAFT 19의 종결 판단, 허용 법령 근거를 400자 이내로 종합한다. `학교폭력이 아닌 사안 등 종결처리` 흐름을 유지하고 제13조, 제13조의2, 제14조 계열은 쓰지 않는다. |
