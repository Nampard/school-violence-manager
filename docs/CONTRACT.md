...
PDF 원문은 아래한글 문서를 변환한 참고 자료다. MVP는 모든 HWP 파일을 직접 재작성하지 않고, 사용자가 공문 기록 프로그램이나 서식 파일에 직접 복사 및 붙여넣기 쉬운 문구 블록을 생성한다. `<서식10>`과 DRAFT 18~22 생성문은 존대어를 쓰지 않고 행정문체를 필수로 사용한다.
...
|---|---|---|---|---|---|
| `<서식10>` 사안접수 보고 | 사안접수 | `FORM_10_CASE_INTAKE` | 사용자가 두서없이 적은 `사실 확인내용` | 사안접수 보고용 `발생 경위` | `사안 개요`, `조치 사항`은 제외하고 발생 경위만 복사 가능하게 정리. 행정문체, 존대어 금지, 판단 단정 금지, 1,000자 이내 |
| `<서식12>` 사안조사 보고서 | 조사관 입력 | `FORM_12_INVESTIGATION_REPORT` | 외주 조사관 입력란 | 이후 서식 생성의 핵심 source 저장 | 원문 보존, 수정 이력 관리 |
| `<서식18>` 전담기구 심의결과 보고서 | `SELF_RESOLUTION` | `FORM_18_COMMITTEE_REVIEW_RESULT` | `<서식12>` | `자체해결 또는 심의위원회 요청` | 사안 요약 후 `학교폭력 예방 및 대책에 관한 법률 제13조의2 제1항`에 따른 자체해결 요건을 행정문체로 정리, 500자 이내 |
...
| `<서식19>` 전담기구 심의결과 보고서[종결] | `CLOSURE_*` | `FORM_19_COMMITTEE_CLOSURE_RESULT` | `<서식12>` | `종결 시(오인신고 등)` | 사안 요약 후 선택된 종결 사유와 `학교폭력이 아닌 사안 등 종결처리` 취지로 정리. 제13조/제13조의2/제14조 계열 금지, 오인신고/의심사안은 제2조 제1항, 관련자 성인 등 특정 불가는 제2조 제3항만 근거로 사용, 500자 이내 |
| `<서식20>` 학교장 자체해결 동의서 | `SELF_RESOLUTION` | `FORM_20_SELF_RESOLUTION_CONSENT` | `<서식12>` + `<서식18>` | `보호자 전달 통지서 내용` | `<서식18>`의 제13조의2 제1항 자체해결 판단을 더 완곡한 행정문체로 사안조사 내용 정리, 존대어/요청형/제안 문장 제외, 400자 이내 |
| `<서식21>` 종결처리 동의서 | `CLOSURE_*` | `FORM_21_SELF_RESOLUTION_RESULT` | `<서식12>` + `<서식19>` | `보호자 전달 통지서 내용` | `<서식19>`보다 완곡한 행정문체로 사안조사 내용 정리. 허용 법령 근거 유지, 제13조/제13조의2/제14조 계열/존대어/요청형/제안 문장 제외, 400자 이내 |
| `<서식22>` 학교장 자체해결 결과 보고서 | `SELF_RESOLUTION` | `FORM_22_FINAL_CASE_SUMMARY` | `<서식12>` + `<서식18>` | `자체해결 또는 종결 결과 보고서` | 제13조의2 제1항 자체해결 판단을 포함해 최종 결과 보고서에 붙여넣을 행정문체, 400자 이내 |
| `<서식22>` 학교장 종결 결과 보고서 | `CLOSURE_*` | `FORM_22_FINAL_CASE_SUMMARY` | `<서식12>` + `<서식19>` | `자체해결 또는 종결 결과 보고서` | `학교폭력이 아닌 사안 등 종결처리` 결과 보고 문구. 허용 법령 근거를 포함하고 제13조/제13조의2/제14조 계열 없이 최종 종결 결과 보고서에 붙여넣을 행정문체, 400자 이내 |
...
`generation_options.strictness`는 실제 생성 문구에 반영된다. `STRICT`는 필수 판단 근거 중심의 짧고 단정한 행정문체를 사용하고, `BALANCED`는 같은 기준을 유지하면서 사안 요약과 판단 근거를 조금 더 자연스럽게 풀어 쓴다. 두 모드 모두 존대어와 요청형 문장은 사용하지 않는다.
...
|---|---|---|---|---|
| `FORM_10_CASE_INTAKE` | 사안접수 | 사용자 입력 `statement` | `발생 경위` | 1,000자 이내, 행정문체, 존대어 금지, 판단 단정 금지, 사안 개요/조치 사항 제외 |
| `FORM_12_INVESTIGATION_REPORT` | 조사관 입력 | 조사관 입력 원문 | `사안조사 보고서 원문` | 원문 저장, 후속 생성 source |
| `FORM_18_COMMITTEE_REVIEW_RESULT` | `SELF_RESOLUTION` | `FORM_12_INVESTIGATION_REPORT` | `자체해결 또는 심의위원회 요청` | 500자 이내, 제13조의2 제1항 자체해결 요건 포함 |
| `FORM_18_COMMITTEE_REVIEW_RESULT` | `COMMITTEE_REQUEST` | `FORM_12_INVESTIGATION_REPORT` | `자체해결 또는 심의위원회 요청` | 500자 이내, 심의위원회 요청 근거 포함 |
| `FORM_19_COMMITTEE_CLOSURE_RESULT` | `CLOSURE_*` | `FORM_12_INVESTIGATION_REPORT` | `종결 시(오인신고 등)` | 500자 이내, 선택 종결 사유 포함, `학교폭력이 아닌 사안 등 종결처리` 흐름, 제13조/제13조의2/제14조 계열 금지, 허용 제2조 근거만 사용 |
| `FORM_20_SELF_RESOLUTION_CONSENT` | `SELF_RESOLUTION` | `FORM_12_INVESTIGATION_REPORT` + DRAFT 18 생성문 | `보호자 전달 통지서 내용` | 400자 이내, DRAFT 18의 제13조의2 제1항 자체해결 판단을 더 완곡한 행정문체로 정리, 존대어/제안 문장 제외 |
| `FORM_21_SELF_RESOLUTION_RESULT` | `CLOSURE_*` | `FORM_12_INVESTIGATION_REPORT` + DRAFT 19 생성문 | `보호자 전달 통지서 내용` | 400자 이내, DRAFT 19보다 완곡한 행정문체, 학교폭력이 아닌 사안 등 종결처리 흐름, 허용 제2조 근거 유지, 제13조/제13조의2/제14조 계열/존대어/제안 문장 제외 |
| `FORM_22_FINAL_CASE_SUMMARY` | `SELF_RESOLUTION` | `FORM_12_INVESTIGATION_REPORT` + DRAFT 18 생성문 | `자체해결 또는 종결 결과 보고서` | 400자 이내, 제13조의2 제1항 자체해결 흐름을 행정문체로 종합 |
| `FORM_22_FINAL_CASE_SUMMARY` | `CLOSURE_*` | `FORM_12_INVESTIGATION_REPORT` + DRAFT 19 생성문 | `자체해결 또는 종결 결과 보고서` | 400자 이내, 학교폭력이 아닌 사안 등 종결처리 결과를 행정문체로 종합, 허용 제2조 근거 유지, 제13조/제13조의2/제14조 계열 제외 |
