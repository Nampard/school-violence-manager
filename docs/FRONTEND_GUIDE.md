...
- `보통`은 같은 기준을 유지하되 사안 요약과 안내 문장을 조금 더 자연스럽게 풀어 생성한다.
...
5. 프론트는 문서 미리보기 패널에 결과를 표시한다.

서술 원칙:

- 입력자의 표현을 그대로 단정하지 않는다.
- 접수 단계 초안이므로 학교폭력 여부나 책임을 최종 판단하지 않는다.
- 발생 일시, 장소, 관련 학생, 주요 행위를 행정문체로 정리한다.
...
|---|---|---|---|---|
| DRAFT 18 / `<서식18>` | `SELF_RESOLUTION` | `<서식12>` | 자체해결 또는 심의위원회 요청 | 사안 요약 후, 양측 간 사과, 신체적/정신적 피해 경미, 지속적/반복적 사안으로 보기 어려운 점, 보복행위가 없었던 점을 포함하고 학교장 자체해결 요건 해당 취지로 마무리 |
...
| DRAFT 19 / `<서식19>` | `CLOSURE_*` | `<서식12>` | 종결 시(오인신고 등) | 사안 요약 후, 선택된 종결 사유와 학교장 종결 처리 해당 취지로 정리 |
| DRAFT 20 / `<서식20>` | `SELF_RESOLUTION` | `<서식12>` + DRAFT 18 | 보호자 전달 통지서 내용 | 사안 요약 후, 보호자 대상 문서에 맞게 자체해결 판단 근거를 완곡하고 중립적으로 재서술 |
| DRAFT 21 / `<서식21>` | `CLOSURE_*` | `<서식12>` + DRAFT 19 | 보호자 전달 통지서 내용 | 사안 요약 후, 종결 판단 근거를 보호자 대상 문서에 맞게 완곡하게 재서술 |
| DRAFT 22 / `<서식22>` | `SELF_RESOLUTION` | `<서식12>` + DRAFT 18 | 자체해결 또는 종결 결과 보고서 | 사안 요약 후, 자체해결 판단과 결과 보고 흐름을 행정문체로 종합 |
| DRAFT 22 / `<서식22>` | `CLOSURE_*` | `<서식12>` + DRAFT 19 | 자체해결 또는 종결 결과 보고서 | 사안 요약 후, 종결 사유와 결과 보고 흐름을 행정문체로 종합 |
